# -*- coding: utf-8 -*-
"""
Created on Mon Oct  10 17:00:02 2017
...
def color_thresh(img, sat_thresh = (5,100),light_thresh = (200,255), hue_thresh = (15,55)):
    """input img is a RGB, output is single channel B/W image which masks a given color range"""
...
#%% Define a line class to keep track of a history of lines
class Line():
    ...
    def add_new_data(self, xpix_coord, ypix_coord):
        ...
        if len(self.recent_xfitted) == self.n:
            ...
            rad_diff = np.abs(self.radius_of_curvature-findRoadCurvature(self.current_fit))
...
#            self.current_fit.append(np.polyfit(self.ally, self.allx, 2))
        ...
        self.bestx = sum(self.recent_xfitted)/len(left_line.recent_xfitted)
...
#%% 
## test out the line class with an image
import os
...
image_unprocessed = mpimg.imread('../test_images/'+test_images_list[4])

image_shape = (image_unprocessed.shape[1], image_unprocessed.shape[0])

M = cv2.getPerspectiveTransform(src_pts, dst_pts)
Minv = cv2.getPerspectiveTransform(dst_pts, src_pts)


image_processed = processImage(image_unprocessed,(160,255),(230,255),(19,50),(45,255),(0.6,1.3),15)
...
#%%
# Use a moving window to compute initial lane margins
leftlane_ind = []
rightlane_ind = []
...
# find points within margins of each lane
...
def draw_overlay(img, left_fitx, right_fitx):
    """Draw overlay on top of road images given left and right lanes"""
...
def overlayText(img, textString, pos, scale):
    """Draw text over images"""
    cv2.putText(img, textString,(pos[0],pos[1]), font, scale,(255,255,255),2,cv2.LINE_AA)
    


#%%
# This cell implements the image processing pipeline, assumes camera 
...
def process_lane_overlay(imgOrigin):
    """Draw lane overlay over road image"""
    img = np.copy(imgOrigin)
    # undistorts camera lense effects
    image_undistorted = undistort_img(img, objpoints, imgpoints)
    ...
    if left_line.detected == True: 
        ...
        if ((dist_from_center>1) | (np.abs(left_line.radius_of_curvature-right_line.radius_of_curvature)>200)):
            left_line.reset()
            right_line.reset()
    ...
    output_img = draw_overlay(img, left_line.bestx, right_line.bestx) 
...
#%%
ouput_img = process_lane_overlay(image_unprocessed)  

plt.imshow(ouput_img)
plt.show()

#%%
# Create video with lane overlay
from moviepy.editor import VideoFileClip
#from IPython.display import HTML
#
#
#
video_unprocessed = VideoFileClip("../project_video2.mp4")
